...
module.exports = class extends AirPurifier
	.with(MiotApi, Power, Mode, Temperature, Humidity, AQI,
		SwitchableLED, LEDBrightness, Buzzer)
{
	...
	constructor(options) {
		...
		this.defineProperty({did: `${id}`, siid: 2, piid: 2}, {
			name: 'power',
			mapper: (v) => {
				const {siid, piid} = v;
		...
		this.defineProperty({did: `${id}`, siid: 3, piid: 8}, {
			name: 'temperature',
			mapper: (v) => {
				const {siid, piid} = v;
		...
		this.defineProperty({did: `${id}`, siid: 3, piid: 7}, {
			name: 'humidity',
			mapper: (v) => {
				const {siid, piid} = v;
		...
		this.defineProperty({did: `${id}`, siid: 3, piid: 6}, {
			name: 'aqi',
			mapper: (v) => {
				const {siid, piid} = v;
		...
		// Info about usage
		// Filter life remain
		this.defineProperty({did: `${id}`, siid: 4, piid: 3}, {
			name: 'filter1_life',
			mapper: (v) => {
			const {siid, piid} = v;
	...
	changePower(value) {
		const {id} = this.handle.api;
		return this.call('set_properties', [ {did: `${id}`, siid: 2, piid: 2, value} ], {
			refresh: [ 'power', 'mode' ],
			refreshDelay: 200
		});
	}
