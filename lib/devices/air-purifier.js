		...
		SwitchableLED, LEDBrightness, Buzzer, FanSpeed)
{
	...
	constructor(options) {
		...
		this.defineProperty('f1_hour', {
			name: 'filterHours'
		});
		this.defineProperty('rfid_product_id', {
			name: 'rfid_product_id'
		});		
		this.defineProperty('rfid_tag', {
			name: 'rfid_tag'
		});		
	...
	/**
	 * Set volume of sound notifications [0-100]. 0 means off, 100 means max volume
	 */	
	changeVolume(level) {
		return this.call('set_volume', [ level ], { refresh: [ 'volume' ] })
			.then(() => null);
	}
	changeChildLock(level) {
		return this.call('set_child_lock', [ level ? 'on' : 'off' ], { refresh: [ 'child_lock' ] })
			.then(() => null);
	}	
};
