	...
	Parent =>
		class extends Parent.with(Polling) {
						...
						return new Promise(resolve =>
							setTimeout(() => {
								const properties = (Array.isArray(options.refresh)
									...
									.map(prop => {
										if (this._reversePropertyDefinitions[prop]) {
											return this._reversePropertyDefinitions[prop];
										}
									})
			...
			defineProperty(name, def) {
				this._propertiesToMonitor.push(def.name || name);
			...
			setProperty(key, value) {
				...
				if (!isDeepEqual(oldValue, value)) {
					...
					this.propertyUpdated(key, value.value, oldValue);
				}
			}
			...
			loadProperties(props) {
				...
				return this.call('get_properties', properties).then(result => {
					...
					result.forEach((val, i) => {
						const p = props.find(prop => {
							if (
								this._reversePropertyDefinitions[prop] &&
								val &&
								val.siid &&
								val.piid
							) {
								return (
									this._reversePropertyDefinitions[prop].siid === val.siid &&
									this._reversePropertyDefinitions[prop].piid === val.piid
								);
							}
						...
						let value = null;
						if (val.code === 0){ value = val.value; }						

						this._pushProperty(obj, p, value);
					});
					// for (let i = 0; i < result.length; i++) {
					// 	this._pushProperty(
					// 		obj,
					// 		this._propertiesToMonitor[i],
					// 		result[i].value
					// 	);
					// }
					return obj;
