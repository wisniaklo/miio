...
module.exports = Thing.type(Parent => class extends Parent.with(Polling) {
			...
			.then(res => {
				if(options && options.refresh) {
					...
					return new Promise(resolve => setTimeout(() => {
						const properties = (Array.isArray(options.refresh) ? options.refresh : this._propertiesToMonitor).map((prop) => {
	...
	loadProperties(props) {
		...
		return this.call('get_properties', props)
			.then(result => {
				const obj = {};
				for(let i=0; i<result.length; i++) {
					this._pushProperty(obj, this._propertiesToMonitor[i], result[i].value);
				}
				return obj;
			});
	}
